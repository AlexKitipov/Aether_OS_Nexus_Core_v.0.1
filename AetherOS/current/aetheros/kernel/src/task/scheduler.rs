...
static TASKS: Mutex<alloc::collections::BTreeMap<u64, TaskControlBlock>> = Mutex::new(alloc::collections::BTreeMap::new());
...
pub fn init() {
    ...
    TASKS.lock().insert(kernel_task.id, kernel_task.clone());
...
pub fn add_task(task: TaskControlBlock) {
    let task_id = task.id;
    kprintln!("[kernel] scheduler: Adding task '{}' (ID: {}).", task.name, task_id);
...
pub fn block_current_task() {
    ...
    if let Some(task) = tasks.get_mut(&current_id) {
        ...
        kprintln!("[kernel] scheduler: Task '{}' (ID: {}) blocked.", task.name, current_id);
    }
...
pub fn unblock_task(task_id: u64) {
    ...
    if let Some(task) = tasks.get_mut(&task_id) {
        if task.state == TaskState::Blocked {
            ...
            RUN_QUEUE.lock().push_back(task_id);
            kprintln!("[kernel] scheduler: Task '{}' (ID: {}) unblocked.", task.name, task_id);
        }
    }
}
...
pub fn schedule() {
    ...
    if let Some(next_task_id) = run_queue.pop_front() {
        if let Some(next_task) = tasks.get_mut(&next_task_id) {
            next_task.state = TaskState::Running;
            *current_id_guard = next_task_id;
            kprintln!("[kernel] scheduler: Context switch: from {} to {}.", old_task_id, next_task_id);
            // In a real scheduler, actual CPU context switch would occur here.
        } else {
            kprintln!("[kernel] scheduler: ERROR: Next task ID {} not found in TASKS. Rescheduling.", next_task_id);
            // If a task isn't found (e.g., it was removed), try to schedule again.
            schedule();
        }
    } else {
...
pub fn get_current_task_tcb() -> TaskControlBlock {
    ...
    TASKS.lock().get(&current_id).cloned().unwrap_or_else(|| {
        ...
        kprintln!("[kernel] scheduler: WARNING: Current task ID {} not found. Returning dummy task.", current_id);
