...
use crate::syscall::{syscall3, SYS_LOG, SYS_IRQ_REGISTER, SYS_NET_RX_POLL, SUCCESS, E_ERROR, SYS_NET_ALLOC_BUF, SYS_NET_FREE_BUF, SYS_NET_TX, SYS_IRQ_ACK, SYS_GET_DMA_BUF_PTR, SYS_SET_DMA_BUF_LEN, SYS_IPC_RECV_NONBLOCKING};
...
pub extern "C" fn _start() -> ! {
    ...
    unsafe {
        ...
        if res == SUCCESS {
            ...
            log(&alloc::format!("Net-Bridge: Failed to register IRQ 11: {}.", res));
        ...
        if let Ok(Some(irq_event_data)) = own_chan.recv_non_blocking() {
            ...
            if len > SUCCESS {
                ...
                if let Err(e) = set_dma_buffer_len(rx_dma_handle, len as usize) {
                    ...
                    // In a real system, we'd allocate a *new* buffer for the next incoming packet, or recycle one from a pool.
