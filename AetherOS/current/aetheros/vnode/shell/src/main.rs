    ...
    fn handle_request(&mut self, request: ShellRequest) -> ShellResponse {
        match request {
            ShellRequest::ExecuteCommand { command, args } => {
                ...
                log(&alloc::format!(
                    "Shell: Executing command: {} with args: {:?}",
                    command,
                    args
                ));
                ...
                match command.as_str() {
                    ...
                    "ls" => {
                        match self.vfs_chan.send_and_recv::<VfsRequest, VfsResponse>(
                            ...
                            Ok(VfsResponse::Error { message, .. }) => {
                                ShellResponse::Error(format!("ls: {}", message))
                            }
                            _ => ShellResponse::Error(
                                "ls: Unexpected response from VFS".to_string(),
                            ),
                        }
                    }
                    "ping" => {
                        if let Some(hostname) = args.get(0) {
                            match self.dns_chan.send_and_recv::<DnsRequest, DnsResponse>(
                                &DnsRequest::ResolveHostname {
                                ...
                                Ok(DnsResponse::NotFound { query }) => {
                                    ShellResponse::Error(format!(
                                        "ping: Host '{}' not found.",
                                        query
                                    ))
                                }
                                Ok(DnsResponse::Error { message }) => {
                                    ShellResponse::Error(format!("ping: DNS error: {}", message))
                                }
                                _ => ShellResponse::Error(
                                    "ping: Unexpected response from DNS Resolver".to_string(),
                                ),
                            }
                        } else {
                    ...
                    "start" => {
                        if let Some(service_name) = args.get(0) {
                            match self.init_chan.send_and_recv::<InitRequest, InitResponse>(
                                &InitRequest::ServiceStart {
                                    service_name: service_name.clone(),
                                },
                            ) {
                                Ok(InitResponse::Success(msg)) => ShellResponse::Success(msg),
                                Ok(InitResponse::Error(msg)) => {
                                    ShellResponse::Error(format!("start: {}", msg))
                                }
                                _ => ShellResponse::Error(
                                    "start: Unexpected response from Init Service".to_string(),
                                ),
                            }
                        } else {
                    ...
                    _ => ShellResponse::CommandOutput {
                        stdout: format!("Command '{}' not found.\n", command),
                        stderr: String::new(),
                        exit_code: 127,
                    },
                }
            }
            ShellRequest::ChangeDirectory { path } => self.handle_change_directory(path),
        ...
        loop {
            if let Ok(Some(req_data)) = self.client_chan.recv_non_blocking() {
                if let Ok(request) = postcard::from_bytes::<ShellRequest>(&req_data) {
                    ...
                    self.client_chan
                        .send(&response)
                        .unwrap_or_else(|_| log("Shell Service: Failed to send response to client."));
                }
            }
