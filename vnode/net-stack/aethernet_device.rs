...
use crate::syscall::{
    syscall2, syscall3, E_ACC_DENIED, E_ERROR, SUCCESS, SYS_GET_DMA_BUF_PTR, SYS_LOG,
    SYS_NET_ALLOC_BUF, SYS_NET_FREE_BUF, SYS_SET_DMA_BUF_LEN,
};
...
pub fn net_alloc_buf(size: usize) -> Result<u64, u64> {
    unsafe {
        ...
        if handle == E_ERROR || handle == E_ACC_DENIED {
            Err(handle)
        } else {
            Ok(handle)
        }
...
pub fn get_dma_buffer_ptr(handle: u64) -> Result<*mut u8, u64> {
    unsafe {
        ...
        if ptr == E_ERROR || ptr == E_ACC_DENIED {
            Err(ptr)
        } else {
            Ok(ptr as *mut u8)
        }
...
impl<'a> RxToken for PacketRxToken<'a> {
        ...
        F: FnOnce(&mut [u8]) -> R,
    {
        ...
        if let Err(e) = net_free_buf(self.dma_handle) {
            log(&alloc::format!(
                "AetherNetDevice: Failed to free RX DMA buffer: {}",
                e
            ));
        }
        result
...
impl<'a> TxToken for PacketTxToken<'a> {
        ...
        F: FnOnce(&mut [u8]) -> R,
    {
        ...
        net_bridge_chan.send(&msg).unwrap_or_else(|_| {
            log(&alloc::format!(
                "AetherNetDevice: Failed to send TxPacket to net-bridge for handle: {}",
                self.dma_handle
            ))
        });
...
impl<'a> Device<'a> for AetherNetDevice {
    ...
    fn receive(&'a mut self, _timestamp: Instant) -> Option<(Self::RxToken, Self::TxToken)> {
        ...
        if let Some((dma_handle, len)) = self.rx_packet_queue.pop_front() {
            if let Ok(buf_ptr) = get_dma_buffer_ptr(dma_handle) {
                ...
                log(&alloc::format!(
                    "AetherNetDevice: Failed to get buffer pointer for RX DMA handle {}. Freeing it.",
                    dma_handle
                ));
                // Free the DMA buffer if ptr is invalid
                if let Err(e) = net_free_buf(dma_handle) {
                    log(&alloc::format!(
                        "AetherNetDevice: Failed to free RX DMA buffer (ptr error, queue): {}",
                        e
                    ));
                }
                None
    ...
    fn transmit(&'a mut self, _timestamp: Instant) -> Option<Self::TxToken> {
        ...
        let dma_handle = match net_alloc_buf(1536) {
            ...
            Err(e) => {
                log(&alloc::format!(
                    "AetherNetDevice: Failed to alloc TX DMA buffer: {}",
                    e
                ));
                return None;
            }
        ...
        if let Ok(buf_ptr) = get_dma_buffer_ptr(dma_handle) {
            ...
            log(&alloc::format!(
                "AetherNetDevice: Failed to get buffer pointer for TX DMA handle {}. Freeing it.",
                dma_handle
            ));
            if let Err(e) = net_free_buf(dma_handle) {
                log(&alloc::format!(
                    "AetherNetDevice: Failed to free TX DMA buffer (ptr error): {}",
                    e
                ));
            } // Try to free if ptr couldn't be obtained
            None
        }
    }
}
